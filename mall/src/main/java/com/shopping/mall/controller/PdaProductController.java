...
public class PdaProductController {
	...
	public String registerProductInfo(PdaProductDto pdaProductDto, MultipartHttpServletRequest mReq,
			HttpSession session) {
		...
		try {
			...
			for (MultipartFile mFile : uploadedFiles) {
				if (mFile != null && mFile.getSize() > 0) {
					...
					// f.setFileName("/mall/resources/imagefile/" +
					// uniqueFileName);
			...
			pdaProductService.registerProductInfo(pdaProductDto, oriNameList, fileNameList); // product
																								// insert
	...
	public String modifyProduct(PdaProductDto pdaProductDto) {
		...
		pdaProductService.modifyProductInfoByProductNo(pdaProductDto); // product
																		// update
	...
	// 안드로이드 상품 상세 리스트
	@RequestMapping(value = "/product/mproductlist.action", method = { RequestMethod.GET,
	...
	// 안드로이드 선택 상품 카트에 담기
	@RequestMapping(value = "/product/maddselectedproductlisttocart.action", method = { RequestMethod.GET,
			RequestMethod.POST }, produces = "text/plain;charset=UTF-8")
	@ResponseBody
	...
	@ResponseBody
	@RequestMapping(value = "/myorder/morderlist.action", method = { RequestMethod.GET,
			RequestMethod.POST }, produces = "text/plain;charset=UTF-8")
